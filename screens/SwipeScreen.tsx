...
export default function SwipeScreen() {
  ...
  const handleSwipeRight = useCallback(() => { if (current) addToWatchlist(current); advance(); }, [current, addToWatchlist, advance]);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.deck}>
        {!current ? (
          ...
          <>
            {next && (
              <SwipeableCard key={next.id} wine={next}
                onSwipeLeft={() => {}} onSwipeRight={() => {}}
                onSwipeUp={() => {}} onTap={() => {}}
                isTop={false} stackIndex={1} />
            )}
            <SwipeableCard key={current.id} wine={current}
      ...
      <QuantityModal visible={!!qtyWine} wine={qtyWine}
        onConfirm={(qty) => { if (qtyWine) { addToCart(qtyWine, qty); setQtyWine(null); advance(); } }}
        onClose={() => setQtyWine(null)} />

      <WineDetailModal visible={!!detailWine} wine={detailWine}
        onClose={() => setDetailWine(null)}
        onAddToWatchlist={(w) => addToWatchlist(w)}
        onAddToCart={(w) => { setDetailWine(null); setTimeout(() => setQtyWine(w), 320); }}
...
const styles = StyleSheet.create({
  ...
  actions: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: spacing.lg, paddingVertical: spacing.md, paddingHorizontal: spacing.lg },
  actionBtn: { width: 64, height: 64, borderRadius: 32, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.card, borderWidth: 1 },
  ...
  resetBtn: { marginTop: spacing.lg, paddingHorizontal: spacing.lg, paddingVertical: spacing.md, borderRadius: radii.lg, borderWidth: 1, borderColor: colors.accent },
