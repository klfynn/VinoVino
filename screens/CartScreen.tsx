  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>{cartCount} {cartCount === 1 ? 'Flasche' : 'Flaschen'}</Text>
      </View>
      ...
      {cart.length === 0 ? (
        ...
        <>
          <FlatList data={cart} keyExtractor={(item) => item.wine.id} contentContainerStyle={styles.list}
            renderItem={({ item }) => (
                ...
                <View style={styles.info}>
                  ...
                  <View style={styles.bottomRow}>
                    <View style={styles.stepper}>
                      <TouchableOpacity onPress={() => updateCartQuantity(item.wine.id, item.quantity - 1)} style={styles.stepBtn}>
                    ...
                    <Text style={styles.lineTotal}>€ {(item.wine.price * item.quantity).toFixed(2)}</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', backgroundColor: colors.card, borderRadius: radii.lg, borderWidth: 1, borderColor: colors.border, overflow: 'hidden', marginBottom: spacing.md },
  ...
  stepBtn: { width: 32, height: 32, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.cardElevated },
  ...
  footer: { position: 'absolute', left: 0, right: 0, bottom: 0, padding: spacing.lg, backgroundColor: colors.card, borderTopWidth: 1, borderTopColor: colors.border },
  ...
  checkoutBtn: { backgroundColor: colors.accent, paddingVertical: spacing.md, borderRadius: radii.lg, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm },
