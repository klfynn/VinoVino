  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {watchlist.length === 0 ? (
        ...
        <FlatList data={watchlist} keyExtractor={(w) => w.id} contentContainerStyle={styles.list}
          renderItem={({ item }) => (
      ...
      <QuantityModal visible={!!qtyWine} wine={qtyWine}
        onConfirm={(qty) => { if (qtyWine) { addToCart(qtyWine, qty); setQtyWine(null); } }}
...
const styles = StyleSheet.create({
  ...
  ratingBadge: { borderWidth: 1, borderColor: colors.accent, paddingHorizontal: spacing.sm, paddingVertical: 2, borderRadius: radii.sm },
  ...
  cartBtn: { flex: 1, backgroundColor: colors.accent, paddingVertical: 8, borderRadius: radii.md, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  cartBtnText: { color: colors.background, fontWeight: '700', fontSize: 12, letterSpacing: 0.5 },
  removeBtn: { width: 36, height: 36, borderRadius: radii.md, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' },
